...
class BasePage(object):
    ...
    def openmax(self, url):
        '''
        使用get打开url后，最大化窗口，判断title符合预期
        '''
        self.driver.get(url)
        self.driver.maximize_window()
    def opensize(self,url,size):
        '''使用get打开url后，可调控窗口大小'''
        self.driver.get(url)
    ...
    def is_located(self, locator, timeout=10):
        '''判断元素被定位到（并不意味着可见），定为到返回element,没定位到返回False'''
...
if __name__ == '__main__':
    driver = webdriver.Chrome()    # 实例1 driver
    base_driver = BasePage(driver)  # 实例2 base_driver
    size1 = (480,960)
    base_driver.opensize("https://m.wook.id",size1)
    input_loc = ("link text","Login")
    a= base_driver.get_text(input_loc)
    print(a)
    # button_loc = ("id", "su")
